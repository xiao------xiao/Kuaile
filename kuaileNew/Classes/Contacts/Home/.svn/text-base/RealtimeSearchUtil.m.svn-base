...
@property (nonatomic) SEL selector1;
@property (nonatomic) SEL selector2;
...
@synthesize selector1 = _selector1;
@synthesize selector2 = _selector2;
    ...
    dispatch_async(self.searchQueue, ^{
        if (string.length == 0) {
            ...
            for (id object in weakSelf.source) {
                NSString *tmpString = @"";
                if (weakSelf.selector1) {
                    if([object respondsToSelector:weakSelf.selector1]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                        tmpString = [[object performSelector:weakSelf.selector1] lowercaseString];
#pragma clang diagnostic pop
                ...
                // 先搜索 selector1 返回的搜索源
                if([tmpString rangeOfString:subStr].location != NSNotFound) {
                    [results addObject:object];
                } else {  // 没有再搜索 selector2 返回的搜索源
                    if (weakSelf.selector2) {
                        if([object respondsToSelector:weakSelf.selector2]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                            tmpString = [[object performSelector:weakSelf.selector2] lowercaseString];
#pragma clang diagnostic pop
                        }
                    } else {
                        continue;
 ...
 *  @param resultBlock 回调方法，返回搜索结果
 */
- (void)realtimeSearchWithSource:(id)source searchText:(NSString *)searchText collationStringSelector:(SEL)selector selector2:(SEL)selector2 resultBlock:(RealtimeSearchResultsBlock)resultBlock
{
    if (!source || !searchText || !resultBlock) {
        if (resultBlock) {
            _resultBlock(source);
