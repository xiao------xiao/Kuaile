...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) { // 保存
        id <MWPhoto> currentPhoto = [self photoAtIndex:index];
        UIImageWriteToSavedPhotosAlbum([currentPhoto underlyingImage], self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
    }
}

- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"didSaveImageFailure" object:self];
    } else {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"didSaveImageSuccess" object:self];
    }
}
 */
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    [self.activityViewController setCompletionHandler:^(NSString *activityType, BOOL completed) {
                        if ([activityType containsString:@"SaveToCameraRoll"]) {
                            [mNotificationCenter postNotificationName:@"didSaveImageSuccess" object:nil];
                        }
