...
//            EMBuddy *buddy = (EMBuddy *)object;
            XYRecommendFriendModel *buddy = (XYRecommendFriendModel *)object;
...
//            EMBuddy *buddy2 = (EMBuddy *)object2;
            ...
            XYRecommendFriendModel *buddy1 = (XYRecommendFriendModel *)object1;
            XYRecommendFriendModel *buddy2 = (XYRecommendFriendModel *)object2;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    XYRecommendFriendModel *buddy = [[_dataSource objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    cell.usernameNick = buddy.nickname.length > 0 ? buddy.nickname : buddy.username;
//    cell.imageView.layer.cornerRadius = 20;
    UIImage *placeholderImage = TZPlaceholderAvaterImage;
    [cell.imageView sd_setImageWithURL:TZImageUrlWithShortUrl(buddy.avatar) placeholderImage:placeholderImage options:SDWebImageRefreshCached];
    return cell;
}
    ...
    [[RealtimeSearchUtil currentUtil] realtimeSearchWithSource:self.contactsSource searchText:searchText collationStringSelector:@selector(username) resultBlock:^(NSArray *results) {
        if (results) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (XYRecommendFriendModel *buddy in results) {
                    if ([weakSelf.selectedContacts containsObject:buddy])
                    {
...
- (void)reloadFooterView
{
    ...
    for (int i = 0; i < count; i++) {
        XYRecommendFriendModel *buddy = [self.selectedContacts objectAtIndex:i];
...
//        remarkView.remark = buddy.username;
        [remarkView.imageView sd_setImageWithURL:buddy.avatar placeholderImage:TZPlaceholderAvaterImage options:SDWebImageRefreshCached];
        remarkView.remarkLabel.text = buddy.nickname;
...
//    }
    ...
    if (_memberDatas.count > 0) {
        [_dataSource addObjectsFromArray:[self sortRecords:_memberDatas]];

//        _dataSource = [_memberDatas copy];
        [self hideHud];
...
- (void)doneAction:(id)sender
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(viewController:didFinishSelectedSources:)]) {
        if ([_blockSelectedUsernames count] == 0) {
            ...
            for (XYRecommendFriendModel *buddy in self.selectedContacts) {
