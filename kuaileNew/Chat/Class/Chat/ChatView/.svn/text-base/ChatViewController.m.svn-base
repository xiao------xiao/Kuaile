...
@property (nonatomic, strong) XYChatTipView *chatTipView;
...
- (instancetype)initWithChatter:(NSString *)chatter isAttented:(BOOL)isAttented isGroup:(BOOL)isGroup
{
    ...
    if (self) {
        if (!isGroup) {
            [self.view addSubview:self.chatTipView];
            // 判断是否屏蔽
            NSArray *blockList = [[EaseMob sharedInstance].chatManager blockedList];
            if ([blockList containsObject:chatter]) { // 已屏蔽
                [self setIsScreenStatus];
            } else {
                [self setNoScreenStatus];
            }
            
            // 判断是否关注
            if (isAttented) {
                [self setIsAttentedStatus];
            } else {
                [self setNoAttentedStatus];
            }
            CGRect viewFrame = self.view.frame;
            _chatTipViewY = CGRectGetMaxY(self.chatTipView.frame);
            viewFrame.origin.y = _chatTipViewY;
            viewFrame.size.height = self.view.frame.size.height - _chatTipViewY - 64;
...
- (instancetype)initWithChatter:(NSString *)chatter conversationType:(EMConversationType)type
{
    ...
    if (self) {
        ...
        if ([chatter isEqualToString:@"ttouch"]) {
...
- (void)setIsScreenStatus {
    self.chatTipView.screenBtn.selected = YES;
    [self.chatTipView.screenBtn setTitle:@"已屏蔽" forState:UIControlStateNormal];
    [self.chatTipView.screenBtn setBackgroundColor:[UIColor whiteColor]];
    [self.chatTipView.screenBtn setTitleColor:TZColorRGB(150) forState:0];
    [self.chatTipView.screenBtn.layer setBorderWidth:0.7];
}

- (void)setNoScreenStatus {
    self.chatTipView.screenBtn.selected = NO;
    [self.chatTipView.screenBtn setTitle:@"屏蔽" forState:UIControlStateNormal];
    [self.chatTipView.screenBtn setBackgroundColor:TZColor(45, 178, 249)];
    [self.chatTipView.screenBtn setTitleColor:[UIColor whiteColor] forState:0];
    [self.chatTipView.screenBtn.layer setBorderWidth:0];
}

- (void)setIsAttentedStatus {
    self.chatTipView.attentBtn.selected = YES;
    [self.chatTipView.attentBtn setTitle:@"已关注" forState:UIControlStateNormal];
    [self.chatTipView.attentBtn setBackgroundColor:[UIColor whiteColor]];
    [self.chatTipView.attentBtn setTitleColor:TZColorRGB(150) forState:0];
    [self.chatTipView.attentBtn.layer setBorderWidth:0.7];
}

- (void)setNoAttentedStatus {
    self.chatTipView.attentBtn.selected = NO;
    [self.chatTipView.attentBtn setTitle:@"+关注" forState:UIControlStateNormal];
    [self.chatTipView.attentBtn setBackgroundColor:TZColor(45, 178, 249)];
    [self.chatTipView.attentBtn setTitleColor:[UIColor whiteColor] forState:0];
    [self.chatTipView.attentBtn.layer setBorderWidth:0];
}
...
- (void)setupBarButtonItem {
    NSString *se = self.titleText;
    if (![self.titleText isEqualToString:@"开心客服"]) {
        if (self.isChatGroup) {
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"group_detail"] style:UIBarButtonItemStyleDone target:self action:@selector(rightBarButtonItemClick)];
        } else {
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"yhxx"] style:UIBarButtonItemStyleDone target:self action:@selector(rightBarButtonItemClick)];
        }
    }
}
...
- (XYChatTipView *)chatTipView {
    if (!_chatTipView) {
        _chatTipView = [[XYChatTipView alloc] init];
        _chatTipView.frame = CGRectMake(0, 0, mScreenWidth, 60);
        _chatTipView.backgroundColor = [UIColor whiteColor];
        MJWeakSelf
        [_chatTipView setDidClickScreenBtnBlock:^(BOOL selected){
            if (selected) { // 屏蔽
                EMError *error = [[EaseMob sharedInstance].chatManager blockBuddy:weakSelf.chatter relationship:eRelationshipBoth];
            } else {
                EMError *error = [[EaseMob sharedInstance].chatManager unblockBuddy:weakSelf.chatter];
            }
        }];
        
        [_chatTipView setDidClickAttentionBtnBlock:^(BOOL selected){ //
            [mNotificationCenter postNotificationName:@"handAttentionNoti" object:nil];
            [weakSelf checkAttentionStatusWithBuddyName:weakSelf.chatter];
        }];
    }
    return _chatTipView;
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        ...
        CGFloat height = [EMChatViewCell tableView:tableView heightForRowAtIndexPath:indexPath withObject:(MessageModel *)obj];
        MessageModel *model = (MessageModel *)obj;
        if (model.isGM) {
            height += 40;
        }
...
//    self.imagePicker.mediaTypes = @[(NSString *)kUTTypeImage,(NSString *)kUTTypeMovie];
    self.imagePicker.mediaTypes = @[(NSString *)kUTTypeImage]; // 隐藏视频
    [self presentViewController:self.imagePicker animated:YES completion:NULL];
    self.isInvisible = YES;
#endif
}
...
{
    ICELoginUserModel *userModel = [ICELoginUserModel sharedInstance];
    NSDictionary *ext = @{@"nickname" : userModel.nickname};
...
- (void)didChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if (!_isChatGroup) {
            rect.origin.y = _chatTipViewY;
            rect.size.height = self.view.frame.size.height - toHeight - _chatTipViewY;
        } else {
            rect.origin.y = 0;
...
#warning 陈冰 20151113 昵称
            NSString *strUserName = DEF_PERSISTENT_GET_OBJECT(DEF_USERNAME);
            NSDictionary *dict = message.ext;
            if (dict) {
                model.nickName = [dict objectForKey:@"nickname"];
                if (![model.username isEqualToString:strUserName]) {
                    model.isGM = [[dict objectForKey:@"remind"] isEqualToString:@"1"] ? YES : NO;
                }
            }
...
- (void)showRoomContact:(id)sender
{
    ...
    if (self.conversationType == eConversationTypeGroupChat) {
        ICEGroupInfoViewController *groupInfo = [[ICEGroupInfoViewController alloc] initWithNibName:@"ICEGroupInfoViewController" bundle:[NSBundle mainBundle] groupID:_chatter];
...
-(void)sendTextMessage:(NSString *)textMessage
{
    ...
    ICELoginUserModel *userModel = [ICELoginUserModel sharedInstance];
    NSString *isMG = [ICETools validateBankCardNumber:textMessage] ? @"1" : @"";
    NSDictionary *ext = @{
...
- (void)rightBarButtonItemClick {
    if (!self.isChatGroup) {
        XYChatSettingController *chatSettingVc = [[XYChatSettingController alloc] init];
        chatSettingVc.name = self.conversation.chatter;
        MJWeakSelf
        [chatSettingVc setDidHandleScreenBlock:^(BOOL isScreen){
            if (isScreen) { // 已屏蔽
                [weakSelf setIsScreenStatus];
            } else {
                [weakSelf setNoScreenStatus];
            }
        }];
        [chatSettingVc setDidHandleAttentionBlock:^(BOOL isAttented){
            if (isAttented) { // 已关注
                [weakSelf setIsAttentedStatus];
            } else {
                [weakSelf setNoAttentedStatus];
            }
        }];
        [self.navigationController pushViewController:chatSettingVc animated:YES];
    } else {
        XYGroupInfoViewController *groupInfoVc = [[XYGroupInfoViewController alloc] initWithGroupId:_chatter];
        if (self.groupInfoModel) {
            groupInfoVc.model = self.groupInfoModel;
        }
        groupInfoVc.pushFromChatVc = YES;
        groupInfoVc.gid = self.gid;
