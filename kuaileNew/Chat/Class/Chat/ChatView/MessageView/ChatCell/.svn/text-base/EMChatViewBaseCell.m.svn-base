...
//    UIImage *placeholderImage = [UIImage imageNamed:@"chatListCellHead"];
    ...
    if ([messageModel.username isEqualToString:[[EaseMob sharedInstance].chatManager loginInfo][@"username"]]) {
        [self.headImageView sd_setImageWithURL:[NSURL URLWithString:[ICELoginUserModel sharedInstance].avatar] placeholderImage:placeholderImage options:EMSDWebImageRefreshCached];
    }else if ([messageModel.username isEqualToString:@"ttouch"]) {
        self.headImageView.image = [UIImage imageNamed:@"Icon-76"];
//        [self.headImageView sd_setImageWithURL:[NSURL URLWithString:[ICELoginUserModel sharedInstance].avatar] placeholderImage:placeholderImage options:EMSDWebImageRefreshCached];
    }else{
        ...
        if (messageModel.headImageURL) {
            ...
            [TZHttpTool postWithURL:ApiDeletefetAvatar params:@{@"usernames" : messageModel.username} success:^(NSDictionary *result) {
                NSArray *datas = result[@"data"];
                if (datas.count > 0) {
                    NSURL  * imageURL = [NSURL URLWithString:result[@"data"][0][@"avatar"]];
                    [self.headImageView sd_setImageWithURL:imageURL placeholderImage:placeholderImage];
                }
