...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        if (_isTS == nil) {
            _isTS = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
            _isTS.textAlignment = NSTextAlignmentCenter;
            _isTS.text = @"内容可能涉及财产信息，请注意保护隐私，以防被骗！";
            _isTS.textColor = [UIColor whiteColor];
            _isTS.backgroundColor = [UIColor lightGrayColor];
            _isTS.font = [UIFont systemFontOfSize:11];
            _isTS.layer.cornerRadius = 5;
            _isTS.layer.masksToBounds = YES;
            _isTS.hidden = YES;
            [self addSubview:_isTS];
        }
...
-(void)layoutSubviews
{
    ...
    if (self.model.isGM) {
        _isTS.hidden = NO;
        _isTS.frame = CGRectMake(-30, self.textLabel.bottom + 20, [UIScreen mainScreen].bounds.size.width - 40, 30);
    } else {
        _isTS.hidden = YES;
    }
}
