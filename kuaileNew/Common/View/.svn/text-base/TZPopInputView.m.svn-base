...
- (instancetype)initWithContentView:(UIView *)contentView {
    ...
    if (self) {
        ...
        // 初始化设置
        self.frame = CGRectMake(20 + mScreenWidth, 100 - 64, mScreenWidth - 40, 320);
...
- (void)awakeFromNib {
    ...
    self.titleLable.textColor = TZMainColor;
    [self.okButton setBackgroundColor:TZMainColor];
...
//            CGPoint center = self.center;
//            center.y = mScreenHeight / 2 - 64;
//            self.center = center;
//        // 键盘上来了
//        } else {
//            CGRect frame = self.frame;
//            frame.origin.y = keyboardF.origin.y - frame.size.height - 2 - 64; // 2是输入框和键盘的间隙，自己调..
//            self.frame = frame;
//        }
//    }];
}
...
- (void)checkTextFieldText {
    ...
    if ([self.titleLable.text rangeOfString:@"密码"].location != NSNotFound) {
        length2 = length3 = 7;
    } else if ([self.titleLable.text rangeOfString:@"联系"].location != NSNotFound) {
        length1 = 10;
    } else if ([self.titleLable.text rangeOfString:@"号码"].location != NSNotFound) {
        length1 = 10;
    }
...
- (BOOL)checkTextFiledData {
    ...
    } else if ([self.titleLable.text isEqualToString:@"修改手机号"] || [self.titleLable.text rangeOfString:@"联系"].location != NSNotFound) {
        // 检测是否是手机号
    ...
    } else if ([self.titleLable.text rangeOfString:@"身份证"].location != NSNotFound) {
        // 检测身份证
        if (![CommonTools isIdentityCard:self.textFiled1.text]) {
            [self.okButton setTitle:@"身份证号格式不正确" forState:UIControlStateDisabled];
            self.okButton.enabled = NO;
            return NO;
        }
    }
