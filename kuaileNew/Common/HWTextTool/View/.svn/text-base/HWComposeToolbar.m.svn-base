...
- (void)setType:(HWComposeToolbarType)type {
    _type = type;
    // 初始化按钮
    if (type == HWComposeToolbarTypeDefault) {
...
//        [self setupBtn:@"compose_camerabutton_background" highImage:@"compose_camerabutton_background_highlighted" type:HWComposeToolbarButtonTypeCamera];
    ...
    if (type != HWComposeToolbarTypeSign) {
...
#pragma mark -- orCode1
    ...
    // 在转发类型下，加一个发送按钮
    if (type == HWComposeToolbarTypeReport) {
        UIButton *btn = [[UIButton alloc] init];
        [btn setTitle:@"发送" forState:0];
//        [btn setImage:[UIImage imageNamed:@"compose_emoticonbutton_background"] forState:0];
        [btn setTitleColor:TZMainColor forState:0];
        btn.titleLabel.font = [UIFont systemFontOfSize:17];
        [btn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
        btn.tag = HWComposeToolbarButtonTypeSend;
        [self addSubview:btn];
    }
}
