...
//        [sendButton setImage:[UIImage imageNamed:@"compose_emotion_delete_highlighted"] forState:UIControlStateHighlighted];
//        [sendButton setImage:[UIImage imageNamed:@"compose_emotion_delete"] forState:UIControlStateNormal];
...
- (HWEmotionButton *)emotionButtonWithLocation:(CGPoint)location
{
    ...
    for (int i = 0; i<count; i++) {
        HWEmotionButton *btn = self.subviews[i + 2];
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i<count; i++) {
        UIButton *btn = self.subviews[i + 2];
        btn.width = btnW;
        btn.height = btnH;
    ...
    self.deleteButton.x = self.width - inset - btnW *2;
    
    // 发送按钮
    self.sendButton.width = btnW;
    self.sendButton.height = btnH;
    self.sendButton.y = self.height - btnH;
    self.sendButton.x = self.width - btnW - inset;
}
...
- (void)sendClick
{
    [HWNotificationCenter postNotificationName:@"HWEmotionDidSendNotification" object:nil];
}
