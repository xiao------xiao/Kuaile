...
- (void)presentFromImageView:(UIView *)fromView
    ...
    if (item.thumbClippedToTop) {
        ...
        NSLog(@"%@\n %@\n %@\n %@\n\n",NSStringFromCGRect(fromFrame),NSStringFromCGRect(_fromView.frame),NSStringFromCGRect(cell.imageContainerView.frame),NSStringFromCGRect(cell.frame));
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    [UIView animateWithDuration:animated ? 0.2 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (isFromImageClipped) {
            ...
            if (_type) {
                if (_type == YYPhotoGroupViewGoodsType) {
                    fromFrame = [fromView convertRect:CGRectMake(0, 64, __kScreenWidth, __kScreenWidth*9/16) toView:cell.imageContainerView];
                } else if (_type == YYPhotoGroupViewTravelType) {
                    fromFrame = [fromView convertRect:CGRectMake(0, 114, __kScreenWidth, __kScreenWidth*9/16) toView:cell.imageContainerView];
                } else if (_type == YYPhotoGroupViewInfoType) {
                    fromFrame = [fromView convertRect:CGRectMake(0, 64, __kScreenWidth, __kScreenWidth*9/16) toView:cell.imageContainerView];
                }
            } else {
