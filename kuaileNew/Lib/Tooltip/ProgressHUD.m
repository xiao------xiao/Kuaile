 ...
 
{
	...
	[self hudSize];
    if (!spin && img == nil && status != nil) {
        UIFont *font = HUD_STATUS_FONT;
        NSDictionary * dict=[NSDictionary dictionaryWithObject: font forKey:NSFontAttributeName];
        CGRect rect=[status boundingRectWithSize:CGSizeMake(250 ,CGFLOAT_MAX) options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:dict context:nil];
        hud.bounds = CGRectMake(0, 0, rect.size.width + 20, rect.size.height + 20);
        label.frame = CGRectMake(10, 10, rect.size.width, rect.size.height);
 ...
 
{
	...
	if (image == nil)
	{
        ...
        image.bounds = CGRectMake(0, 0, 32, 32);
        image.center = CGPointMake(50, 10);
 ...
 
{
	@autoreleasepool
	{
		...
		[NSThread sleepForTimeInterval:HUD_DELAY];
